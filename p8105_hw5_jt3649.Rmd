...
knitr::kable(head(birthday_results))
```
...
The plot shows a strong positive relationship between effect size and power. As the effect size increases, the probability of rejecting the null hypothesis when it is false also increases. This is because larger effect size leads to larger t-statistic, which move the distribution of the sample mean further away from the null value. More extreme t-values will result in more rejections and increasing the power. 
...
- No. The sample avg of 𝜇̂ among simulations where H0 is rejected does not approx. equal to the true value of mu due to selection bias.
